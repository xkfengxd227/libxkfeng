...
bool _input_parameters(int argc, char *argv[], int *alg, char *dsname, int *K, int *h, int *g, int *nq, int *k){
									/// ### self-define ###: parameter list
	/* check parameter numbers */
	if(argc < 2){
		printf("get parameter error: insufficient parameter numbers \n");
		printf("ANC: \n\t-ds dsname \n\t-K number of clusters \n\t-h clustering depth \n\t-g rank range of neighbor clusters \n\t-nq number of queries \n\t-k [k]nn\n");
		return false;
	}
	...
	while(i < argc){
		...
		if(PNAME == get_para_type(argv[i])){		/* a parameter name */
			...
			else if(strcmp(pname, "alg") == 0){		*alg = atoi(argv[i]);	}
