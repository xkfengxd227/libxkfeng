...
#define 	LogFolder	"log"					/// store process log
...
#define 	Algorithm_Search_TrueLB 		11			/// search with true lower bound in HB
#define 	Algorithm_Search_CrossLB 		12			/// search with cross point lower bound in HB
 ...
 *      including I/O and cpu runtime, etc.,
 */
...
/**
 *	calculate distance between a query [q] and a set of vectors [vs], stored in dis
 */
void odis_query_dataset(const float *vs, const float *q, int d, int n, float *dis);

/*********************************** operations on hyperplane *******************************/
/**
 *	distance between query and the crosspoint of \vec{q,c} to Hyperplane(c,oc)
 *	notice: we have figure out the distance between q and c
 */
