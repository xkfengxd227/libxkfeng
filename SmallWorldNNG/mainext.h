...
#define USAGE_STR       "(-log 1 to report)\n\t\t-ds\t[dsname]\n\t\t-alg\t[algorithm indicator]\n\t\t-K\t[K-NNG]\n\t\t-nq\t[no. of queries]\n\t\t-k\t[k-NN]"

/// ### self-define ###: parameter list
bool _input_parameters(int argc, char *argv[], char *dsname, char *alg, int *K, int *nq, int *k, int *logOn){
	...
	while(i < argc){
		...
		if(PNAME == get_para_type(argv[i])){		/* a parameter name */
			...
			else if(strcmp(pname, "alg") == 0){		strcpy(alg, argv[i]);	}
